...
class myallocator {
 ...
 public:
  ...
  char* _Charalloc(size_type _N) { return (_MyAllocate((difference_type)_N, (char*)0)); }

  void deallocate(void* _P, size_type) { operator delete(_P); }

  void construct(pointer _P, const _Ty& _V) { _myConstruct(_P, _V); }

  void destroy(pointer _P) { _myDestory(_P); }
...
inline bool operator==(const myallocator<_Ty>&, const myallocator<_U>&) {
  return (true);
}
template <class _Ty, class _U>
inline bool operator!=(const myallocator<_Ty>&, const myallocator<_U>&) {
